...
const ROOM_CAPACITIES = { sharing: 6, quint: 5, quad: 4, triple: 3, double: 2, private: 1 };
...
const UmrahBookingPage = ({ packageData, flights = [], airlines = [], onBack }) => {
  ...
  const availableRooms = useMemo(() => {
    ...
    // private room derived from double — show alongside if double is available
    if (rooms.includes('double') && !rooms.includes('private')) rooms.push('private');
    return rooms;
  }, [packageData]);

  // private room price = double_selling x 2 (1 person pays for both beds)
  const privateRoomPrice = useMemo(() =>
    (packageData.package_prices?.double?.selling || 0) * 2
  , [packageData]);
  ...
  const grandTotal = useMemo(() =>
    passengers.reduce((sum, p) => sum + priceFor(p.type, p.roomType), 0)
  , [passengers, priceFor]);
  ...
  /* â”€â”€ create booking on backend â”€â”€ */
  const createBooking = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        package_details: packageData,  // full package object (includes flight, hotels, transport, prices)
        rooms_selected,
        ...
        payment_method: null, payment_status: null, payment_details: null
      };
        ...
        <div className="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-slate-100">
          <div className="flex flex-col lg:flex-row lg:items-center gap-6">
            ...
            <div className="text-right shrink-0">
              ...
              <p className="text-2xl font-black text-blue-600">{PKR(grandTotal)}</p>
              <p className="text-[10px] font-bold text-slate-400">{passengers.length} pax</p>
            </div>
          </div>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs transition-all ${
            complete ? 'bg-green-100 text-green-600' : 'bg-blue-50 text-blue-600'
          }`}>
...
const StepTwoReview = ({ passengers, packageData, grandTotal, priceFor, childNoBedPrice, infantPriceVal, isLoading, onEdit, onNext }) => {
...
const StepThreePayment = ({
  ...
  const handleHoldBooking = async () => {
    try {
      ...
      const res   = await fetch(`${API}/api/umrah-bookings/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      if ((paymentMethod === 'bank' || paymentMethod === 'cheque') && paymentData.slipFile) {
        ...
        const r = await fetch(`${API}/api/umrah-bookings/upload-passport`, {
          method: 'POST', headers: { Authorization: `Bearer ${token}` }, body: fd
        });
      ...
      if (paymentMethod === 'credit') {
        ...
        if (!res.ok) { const d = await res.json(); throw new Error(d.detail || 'Failed to update booking'); }
      ...
      // Bank / Cheque / Cash
      const notes = `Payment Method: ${paymentMethod.toUpperCase()} | Amount: PKR ${paymentData.amount} | Date: ${paymentData.date} | ${paymentData.note || 'No additional notes'}` +
        (paymentMethod === 'cash'
          ? ` | Bank: ${paymentData.bankName} | Depositor: ${paymentData.depositorName} | CNIC: ${paymentData.depositorCNIC}`
          : ` | Beneficiary Account: ${paymentData.beneficiaryAccount} | Agent Account: ${paymentData.agentAccount}`);
      ...
      const res = await fetch(`${API}/api/umrah-bookings/${id}`, {
        ...
        body: JSON.stringify({
          ...
          notes,
          payment_details: {
            method: paymentMethod, amount: paymentData.amount,
            date: paymentData.date, bank_name: paymentData.bankName,
            depositor_name: paymentData.depositorName, cnic: paymentData.depositorCNIC,
            beneficiary_account: paymentData.beneficiaryAccount,
            agent_account: paymentData.agentAccount,
            slip_path: slipPath, note: paymentData.note
          }
        })
      });
      if (!res.ok) { const d = await res.json(); throw new Error(d.detail || 'Failed to confirm booking'); }
